...
export class carsModel {
    ...
    getCarTypsbyNameModelGear(search:any): void {
        let apiUrl:string=`http://localhost:55860/api/GetsearchType?name=${search.ManufacturerName}&&model=${search.Model}&&gear=${search.Gear}`;
        this.myHttpClient.get(apiUrl)
            .subscribe((x: any) => {this.TypeName.a=x });
    }
